...
const PERMISSION_ACCOUNT = "TODO";
const AUTHORITY = "TODO";
const SUPER_PERMISSION_ACCOUNT = "TODO";
const SUPER_AUTHORITY = "TODO";
...
const main = async () => {
  ...
  if (PERMISSION_ACCOUNT != SUPER_PERMISSION_ACCOUNT) {
    const superPermission = resp[SUPER_PERMISSION_ACCOUNT];
  ...
  const permissionBefore = permissionCodec.decode(
    permissionAccount.before.data
  );
  const permissionAfter = permissionCodec.decode(permissionAccount.after.data);
