 ...
 * 1. Generate cross-chain payload from memo instruction
 ...
 * 6. Show actual execution logs from memo program
 * 
 * Run with: ts-node examples/memo-simulation.ts
 */
...
async function main() {
  console.log("🚀 Generic Cross-Chain Memo Simulation");
  console.log("======================================\n");
  
  // Step 1: Create memo instruction
  const memoMessage = "Cross-chain governance memo from Ethereum via LayerZero!";
  ...
  const result = await simulateCompleteCrossChainInstruction(memoInstruction, config);
